%author: Kevin Yang. PID A08465591
...
% Check whether item X is available at the taqueria Y.
available_at(X,Y) :- taqueria(Y,_,N), isin(X,N).

% Check whether item X is available in more than one place.
multi_available(X) :- bagof(Y,available_at(X,Y), Bag), length(Bag,L), L > 1.

% Check whether person X is working at taqueria Y.
workers(X,Y) :- taqueria(Y,N,_), isin(X,N).

% Check whether person X works at more than one taqueria.
overworked(X) :- bagof(Y,workers(X,Y),Bag), length(Bag,L), L > 1.

%% Add up sum of integers in a list
addup([], 0). 
addup([FirstNumber | RestOfList], Total) :- 
  cost(FirstNumber,I), 
  addup(RestOfList,Total1), 
  Total is I + Total1.

% Check whether the sum of the costs of the ingredients of item X is equal to K
total_cost(X,K) :- ingredients(X,[H|T]), addup([H|T],Cost), K = Cost.

% Check whether X is a member of a list
member(X,[_|R]) :- member(X,R).
member(X,[X|_]).

% Check whether [X|R] is a subset of S.
subset([X|R],S) :- member(X,S), subset(R,S).
subset([],_).

% Check whether item X has all the ingredients listed in L.
has_ingredients(X,L) :- ingredients(X,A), subset(L,A).

% Returns true if [X|R] is not a subset of S.
