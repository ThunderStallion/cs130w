...
class traced(object):
    ...
    def __call__(self,*args,**dargs):
        ...
        darg = darg[:-2]
        arg = arg[:-2]
        print (("| "*traced.__count) + ",- " + self.__name__ + "(" + darg + arg + ")")
        traced.__count += 1 
        try:
	    returnValue = self.__f(*args,**dargs)
	    traced.__count -= 1
	    print (("| "*traced.__count) + "`- " + str(returnValue))
	    return returnValue     
...
class memoized(object):
    ...
    def __call__(self,*args,**dargs):
        dictionary = str(args) + str(dargs)
 
        if dictionary in self.__dictionary:
	  if isinstance(self.__dictionary[dictionary], Exception): 
            raise self.__dictionary[dictionary]
	  else: return self.__dictionary[dictionary]

	else:
	    try:
		returnValue = self.__f(*args,**dargs)
		self.__dictionary[dictionary] = returnValue
		return returnValue

	    except Exception, exception:
		self.__dictionary[dictionary] = exception
		raise exception
